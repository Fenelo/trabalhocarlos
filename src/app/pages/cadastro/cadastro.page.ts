...
imagem: any;
  ...
  submitForm(){
    ...
    if(!this.form_cadastrar.valid){
      this.presentAlert('Album', 'Erro no Cadastro', 'Todos os Campos são Obrigatórios');
      return false;
    }else{
      this.cadastrar();
    }
  ...
  private cadastrar() : void{
      ...
      this.MusicaService.enviarImagem(this.imagem, this.form_cadastrar.value)
      .then(()=>{
        this.loadingCtrl.dismiss();
        this.presentAlert("Musica", "Adiconar", "Musica Salva!");
        this.router.navigate(['/home']);
      })
      .catch((error)=>{
        console.log(error);
        this.loadingCtrl.dismiss();
        this.presentAlert("Album", "Adicionar", "Erro ao salvar a Musica!");
      })
